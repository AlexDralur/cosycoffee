...
def add_to_bag(request, item_id):
    ...
    if ('quantity_250g' in request.POST
        ...
        weight = '250g'
    elif ('quantity_1kg' in request.POST
            and request.POST['quantity_1kg'] != '0'):
        quantity = int(request.POST['quantity_1kg'])
        weight = '1kg'
    elif ('quantity_ac' in request.POST
            and request.POST['quantity_ac'] != '0'):
    ...
    messages.success(
        request,
        f"Added {quantity} of {product.name} ({weight}) to your bag.")
...
def remove_from_bag(request, item_id, weight):
    ...
    if item_id in bag and weight in bag[item_id]:
        ...
        messages.success(
            request, f"Removed {weight} from the item in your bag.")
        ...
        messages.error(
            request,
            f"Weight '{weight}' not found for the item or item not found.")
